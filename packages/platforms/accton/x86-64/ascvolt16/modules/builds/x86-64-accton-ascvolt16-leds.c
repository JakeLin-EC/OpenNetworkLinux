...
static ssize_t show_led(struct device *dev, struct device_attribute *attr,
			char *buf);
static int ascvolt16_led_probe(struct platform_device *pdev);
static int ascvolt16_led_remove(struct platform_device *pdev);
...
struct ascvolt16_led_data *data = NULL;

static struct platform_driver ascvolt16_led_driver = {
	.probe = ascvolt16_led_probe,
	.remove = ascvolt16_led_remove,
	.driver = {
		.name = DRVNAME,
		.owner = THIS_MODULE,
	},
...
enum ascvolt16_led_sysfs_attrs {
	LED_LOC,
	LED_DIAG,
	LED_ALARM,
	LED_FAN,
...
static struct attribute *ascvolt16_led_attributes[] = {
...
static const struct attribute_group ascvolt16_led_group = {
	.attrs = ascvolt16_led_attributes,
};
...
static struct ascvolt16_led_data *ascvolt16_led_update_device(void)
{
	int status = 0;
							...
							char *buf)
{
	...
	data = ascvolt16_led_update_device();
	if (!data->valid) {
		error = -EIO;
		goto exit;
	}
...
static int ascvolt16_led_probe(struct platform_device *pdev)
{
	...
	status = sysfs_create_group(&pdev->dev.kobj, &ascvolt16_led_group);
	if (status)
		goto exit;
...
static int ascvolt16_led_remove(struct platform_device *pdev)
{
	sysfs_remove_group(&pdev->dev.kobj, &ascvolt16_led_group);

	return 0;
}

static int __init ascvolt16_led_init(void)
{
	int ret;

	data = kzalloc(sizeof(struct ascvolt16_led_data), GFP_KERNEL);
	if (!data) {
		ret = -ENOMEM;
		goto alloc_err;
	}
	...
	ret = platform_driver_register(&ascvolt16_led_driver);
	if (ret < 0)
		goto dri_reg_err;
...
dev_reg_err:
	platform_driver_unregister(&ascvolt16_led_driver);
dri_reg_err:
	kfree(data);
alloc_err:
	return ret;
...
static void __exit ascvolt16_led_exit(void)
{
...
MODULE_DESCRIPTION("ascvolt16_led driver");
MODULE_LICENSE("GPL");

module_init(ascvolt16_led_init);
module_exit(ascvolt16_led_exit);