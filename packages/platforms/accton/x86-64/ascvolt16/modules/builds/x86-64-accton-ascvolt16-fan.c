...
static ssize_t show_version(struct device *dev, struct device_attribute *da,
			char *buf);
static int ascvolt16_fan_probe(struct platform_device *pdev);
static int ascvolt16_fan_remove(struct platform_device *pdev);
...
struct ascvolt16_fan_data {
	struct platform_device *pdev;
	struct mutex update_lock;
	char valid; /* != 0 if registers are valid */
	unsigned long last_updated;	/* In jiffies */
	unsigned char ipmi_resp[NUM_OF_FAN * FAN_DATA_COUNT];
	unsigned char ipmi_resp_cpld[2];
	struct ipmi_data ipmi;
	unsigned char ipmi_tx_data[3];  /* 0: FAN id, 1: 0x02, 2: PWM */
};

struct ascvolt16_fan_data *data = NULL;

static struct platform_driver ascvolt16_fan_driver = {
	.probe = ascvolt16_fan_probe,
	.remove = ascvolt16_fan_remove,
	.driver = {
		.name = DRVNAME,
		.owner = THIS_MODULE,
	},
...
enum ascvolt16_fan_sysfs_attrs {
	FAN_ATTR(1),
	FAN_ATTR(2),
	FAN_ATTR(3),
	NUM_OF_FAN_ATTR,
...
static struct attribute *ascvolt16_fan_attributes[] = {
	/* fan attributes */
	DECLARE_FAN_ATTR(1),
	DECLARE_FAN_ATTR(2),
	DECLARE_FAN_ATTR(3),
...
static const struct attribute_group ascvolt16_fan_group = {
	.attrs = ascvolt16_fan_attributes,
};
...
static struct ascvolt16_fan_data *ascvolt16_fan_update_device(void)
{
	int status = 0;
							...
							char *buf)
{
	...
	data = ascvolt16_fan_update_device();
	if (!data->valid) {
		error = -EIO;
		goto exit;
	}
...
static struct ascvolt16_fan_data *ascvolt16_fan_update_cpld_ver(void)
{
	int status = 0;
								...
								char *buf)
{
	...
	data = ascvolt16_fan_update_cpld_ver();
	if (!data->valid) {
		error = -EIO;
		goto exit;
	}
...
static int ascvolt16_fan_probe(struct platform_device *pdev)
{
	...
	status = sysfs_create_group(&pdev->dev.kobj, &ascvolt16_fan_group);
	if (status)
		goto exit;
...
static int ascvolt16_fan_remove(struct platform_device *pdev)
{
	sysfs_remove_group(&pdev->dev.kobj, &ascvolt16_fan_group);

	return 0;
}

static int __init ascvolt16_fan_init(void)
{
	int ret;

	data = kzalloc(sizeof(struct ascvolt16_fan_data), GFP_KERNEL);
	if (!data) {
		ret = -ENOMEM;
		goto alloc_err;
	}
	...
	ret = platform_driver_register(&ascvolt16_fan_driver);
	if (ret < 0)
		goto dri_reg_err;
...
dev_reg_err:
	platform_driver_unregister(&ascvolt16_fan_driver);
dri_reg_err:
	kfree(data);
alloc_err:
	return ret;
...
static void __exit ascvolt16_fan_exit(void)
{
...
MODULE_DESCRIPTION("ascvolt16_fan driver");
MODULE_LICENSE("GPL");

module_init(ascvolt16_fan_init);
module_exit(ascvolt16_fan_exit);
