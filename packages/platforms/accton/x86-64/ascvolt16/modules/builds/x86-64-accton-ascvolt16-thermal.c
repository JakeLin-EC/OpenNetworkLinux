...
#define DRVNAME "ascvolt16_thermal"
...
static ssize_t show_temp(struct device *dev, struct device_attribute *attr,
	char *buf);
static int ascvolt16_thermal_probe(struct platform_device *pdev);
static int ascvolt16_thermal_remove(struct platform_device *pdev);
...
struct ascvolt16_thermal_data {
	struct platform_device *pdev;
	struct mutex update_lock;
	char valid;		   /* != 0 if registers are valid */
	unsigned long last_updated;	/* In jiffies */
	char   ipmi_resp[15]; /* 3 bytes for each thermal */
	struct ipmi_data ipmi;
	unsigned char ipmi_tx_data[2];  /* 0: thermal id, 1: temp */
};

struct ascvolt16_thermal_data *data = NULL;

static struct platform_driver ascvolt16_thermal_driver = {
	.probe = ascvolt16_thermal_probe,
	.remove = ascvolt16_thermal_remove,
	.driver = {
		.name = DRVNAME,
		.owner = THIS_MODULE,
	},
...
enum ascvolt16_thermal_sysfs_attrs {
	TEMP1_INPUT,
	TEMP2_INPUT,
	TEMP3_INPUT,
	TEMP4_INPUT,
...
static struct attribute *ascvolt16_thermal_attributes[] = {
...
static const struct attribute_group ascvolt16_thermal_group = {
	.attrs = ascvolt16_thermal_attributes,
};
...
static int ascvolt16_thermal_probe(struct platform_device *pdev)
{
	...
	status = sysfs_create_group(&pdev->dev.kobj, &ascvolt16_thermal_group);
	if (status)
		goto exit;
...
static int ascvolt16_thermal_remove(struct platform_device *pdev)
{
	sysfs_remove_group(&pdev->dev.kobj, &ascvolt16_thermal_group);

	return 0;
}

static int __init ascvolt16_thermal_init(void)
{
	int ret;

	data = kzalloc(sizeof(struct ascvolt16_thermal_data), GFP_KERNEL);
	if (!data) {
		ret = -ENOMEM;
		goto alloc_err;
	}
	...
	ret = platform_driver_register(&ascvolt16_thermal_driver);
	if (ret < 0)
		goto dri_reg_err;
...
dev_reg_err:
	platform_driver_unregister(&ascvolt16_thermal_driver);
dri_reg_err:
	kfree(data);
alloc_err:
	return ret;
...
static void __exit ascvolt16_thermal_exit(void)
{
...
MODULE_DESCRIPTION("ascvolt16 Thermal driver");
MODULE_LICENSE("GPL");

module_init(ascvolt16_thermal_init);
module_exit(ascvolt16_thermal_exit);
