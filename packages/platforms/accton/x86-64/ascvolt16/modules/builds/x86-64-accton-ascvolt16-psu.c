...
static ssize_t show_string(struct device *dev, struct device_attribute *attr,
							char *buf);
static int ascvolt16_psu_probe(struct platform_device *pdev);
static int ascvolt16_psu_remove(struct platform_device *pdev);
...
struct ascvolt16_psu_data *data = NULL;

static struct platform_driver ascvolt16_psu_driver = {
	.probe = ascvolt16_psu_probe,
	.remove = ascvolt16_psu_remove,
	.driver = {
		.name = DRVNAME,
		.owner = THIS_MODULE,
	},
...
enum ascvolt16_psu_sysfs_attrs {
	/* psu attributes */
	PSU_ATTR(1),
	PSU_ATTR(2),
	NUM_OF_PSU_ATTR,
...
static struct attribute *ascvolt16_psu_attributes[] = {
	/* psu attributes */
	DECLARE_PSU_ATTR(1),
	DECLARE_PSU_ATTR(2),
	NULL
...
static const struct attribute_group ascvolt16_psu_group = {
	.attrs = ascvolt16_psu_attributes,
};
...
static struct ascvolt16_psu_data *ascvolt16_psu_update_device(struct device_attribute *da)
{
							...
							char *buf)
{
	...
	data = ascvolt16_psu_update_device(da);
	if (!data->valid[pid]) {
		error = -EIO;
		goto exit;
	}
...
static int ascvolt16_psu_probe(struct platform_device *pdev)
{
	...
	status = sysfs_create_group(&pdev->dev.kobj, &ascvolt16_psu_group);
	if (status)
		goto exit;
...
static int ascvolt16_psu_remove(struct platform_device *pdev)
{
	sysfs_remove_group(&pdev->dev.kobj, &ascvolt16_psu_group);
	return 0;
}

static int __init ascvolt16_psu_init(void)
{
	int ret;

	data = kzalloc(sizeof(struct ascvolt16_psu_data), GFP_KERNEL);
	if (!data) {
		ret = -ENOMEM;
		goto alloc_err;
	}
	...
	ret = platform_driver_register(&ascvolt16_psu_driver);
	if (ret < 0)
		goto dri_reg_err;
...
dev_reg_err:
	platform_driver_unregister(&ascvolt16_psu_driver);
dri_reg_err:
	kfree(data);
alloc_err:
	return ret;
...
static void __exit ascvolt16_psu_exit(void)
{
...
MODULE_DESCRIPTION("ascvolt16_psu driver");
MODULE_LICENSE("GPL");

module_init(ascvolt16_psu_init);
module_exit(ascvolt16_psu_exit);
