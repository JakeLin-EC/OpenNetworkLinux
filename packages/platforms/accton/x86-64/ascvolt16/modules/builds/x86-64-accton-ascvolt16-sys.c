...
#define CPU_CPLD_ADDR           0x62
#define FPGA_CPLD_ADDR          0x60
#define MAIN_CPLD_ADDR          0x6a

static void ipmi_msg_handler(struct ipmi_recv_msg *msg, void *user_msg_data);
static int ascvolt16_sys_probe(struct platform_device *pdev);
static int ascvolt16_sys_remove(struct platform_device *pdev);
...
struct ascvolt16_sys_data *data = NULL;

static struct platform_driver ascvolt16_sys_driver = {
	.probe      = ascvolt16_sys_probe,
	.remove     = ascvolt16_sys_remove,
	.driver     = {
		.name   = DRVNAME,
		.owner  = THIS_MODULE,
	},
...
static SENSOR_DEVICE_ATTR(fpga_cpld_ver, S_IRUGO, show_cpld_version, 
			  NULL, FPGA_CPLD_VER);
static SENSOR_DEVICE_ATTR(main_cpld_ver, S_IRUGO, show_cpld_version, 
			  NULL, MAIN_CPLD_VER);

static struct attribute *ascvolt16_sys_attributes[] = {
	&sensor_dev_attr_cpu_cpld_ver.dev_attr.attr,
	&sensor_dev_attr_fpga_cpld_ver.dev_attr.attr,
	&sensor_dev_attr_main_cpld_ver.dev_attr.attr,
	NULL
};

static const struct attribute_group ascvolt16_sys_group = {
	.attrs = ascvolt16_sys_attributes,
};
...
static struct ascvolt16_sys_data *
ascvolt16_sys_update_cpld_ver(unsigned char cpld_addr)
{
	int status = 0;
				 ...
				 struct device_attribute *da, char *buf)
{
	...
	data = ascvolt16_sys_update_cpld_ver(cpld_addr);
	if (!data->valid) {
		error = -EIO;
		goto exit;
	}
...
static int ascvolt16_sys_probe(struct platform_device *pdev)
{
	...
	/* Register sysfs hooks */
	status = sysfs_create_group(&pdev->dev.kobj, &ascvolt16_sys_group);
	if (status) {
		goto exit;
	}
...
static int ascvolt16_sys_remove(struct platform_device *pdev)
{
	...
	sysfs_remove_group(&pdev->dev.kobj, &ascvolt16_sys_group);

	return 0;
}

static int __init ascvolt16_sys_init(void)
{
	int ret;

	data = kzalloc(sizeof(struct ascvolt16_sys_data), GFP_KERNEL);
	if (!data) {
		ret = -ENOMEM;
		goto alloc_err;
	}
	...
	ret = platform_driver_register(&ascvolt16_sys_driver);
...
dev_reg_err:
	platform_driver_unregister(&ascvolt16_sys_driver);
dri_reg_err:
	kfree(data);
alloc_err:
	return ret;
...
static void __exit ascvolt16_sys_exit(void)
{
...
MODULE_DESCRIPTION("ASCVOLT16 System driver");
MODULE_LICENSE("GPL");

module_init(ascvolt16_sys_init);
module_exit(ascvolt16_sys_exit);